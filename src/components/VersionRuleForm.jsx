  ...
  return (
    <div className="flex items-center gap-3">
      <button
        ...
        className={`relative w-11 h-6 rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-slate-800 ${
          checked ? 'bg-amber-500 focus:ring-amber-500' : 'bg-gray-300 dark:bg-slate-600 focus:ring-blue-500'
        }`}
        role="switch"
      ...
      <div>
        <div className="text-gray-800 dark:text-slate-200 text-sm font-medium">{label}</div>
        {description && (
          <div className="text-gray-400 dark:text-slate-500 text-xs mt-0.5">{description}</div>
        )}
      </div>
    </div>
  );
...
export default function VersionRuleForm({ initialData, mode, onSave, onCancel }) {
  ...
  const inputClass = (hasError) =>
    `w-full bg-gray-50 dark:bg-slate-700 border text-gray-900 dark:text-slate-100 rounded-xl px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm placeholder-gray-400 dark:placeholder-slate-500 transition-colors ${
      hasError ? 'border-red-400 dark:border-red-500/70' : 'border-gray-300 dark:border-slate-600'
    }`;

  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl border border-blue-500/30 shadow-xl shadow-blue-900/10">
      {/* Form header */}
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              placeholder="例：3.9"
              className={inputClass(!!errors.fromVersion)}
            />
            {errors.fromVersion && (
          ...
          <div>
            ...
            <input
              ...
              placeholder="例：3.10"
              className={inputClass(!!errors.toVersion)}
            />
            {errors.toVersion && (
              <p className="text-red-500 dark:text-red-400 text-xs mt-1.5 flex items-center gap-1">
                <AlertTriangle size={11} />
        ...
        {/* Migration toggle */}
        <div className="p-4 bg-gray-50 dark:bg-slate-700/40 rounded-xl border border-gray-200 dark:border-slate-700">
          <Toggle
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              type="button"
              onClick={addStep}
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs bg-gray-100 dark:bg-slate-700 hover:bg-gray-200 dark:hover:bg-slate-600 text-gray-600 dark:text-slate-300 rounded-lg transition-colors border border-gray-200 dark:border-slate-600"
            >
              <Plus size={13} />
              新增步驟
            </button>
          ...
          {form.manualSteps.length === 0 ? (
            <div
              onClick={addStep}
              className="flex flex-col items-center justify-center py-8 text-gray-400 dark:text-slate-600 border border-dashed border-gray-300 dark:border-slate-700 rounded-xl cursor-pointer hover:border-gray-400 dark:hover:border-slate-500 hover:text-gray-500 dark:hover:text-slate-500 transition-colors"
            >
            ...
            <div className="space-y-2">
              {form.manualSteps.map((step, index) => (
                <div key={step.id} className="flex items-center gap-2.5">
                  <span className="w-6 h-6 flex-shrink-0 rounded-full bg-gray-100 dark:bg-slate-700 border border-gray-200 dark:border-slate-600 flex items-center justify-center text-xs text-gray-400 dark:text-slate-500 font-mono">
                    {index + 1}
                  </span>
                  <input
                    type="text"
                    ...
                    className="flex-1 bg-gray-50 dark:bg-slate-700 border border-gray-300 dark:border-slate-600 text-gray-900 dark:text-slate-100 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder-gray-400 dark:placeholder-slate-500 transition-colors"
                  />
                  <button
                    type="button"
                    onClick={() => removeStep(step.id)}
                    className="w-8 h-8 flex-shrink-0 flex items-center justify-center text-gray-400 dark:text-slate-600 hover:text-red-500 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors"
                  >
                    <Trash2 size={14} />
                  </button>
                </div>
        ...
        <div className="flex items-center gap-3 pt-2 border-t border-gray-200 dark:border-slate-700">
          ...
          <button
            type="button"
            onClick={onCancel}
            className="flex items-center gap-2 px-5 py-2.5 bg-gray-100 dark:bg-slate-700 hover:bg-gray-200 dark:hover:bg-slate-600 text-gray-600 dark:text-slate-300 rounded-xl text-sm font-medium transition-colors"
          >
            <X size={15} />
            取消
          </button>
